...
El proyecto propone una implementación paralela del cálculo del potencial gravitatorio generado por un conglomerado de partículas en una grilla tridimensional. El cálculo del potencial gravitatorio demanda una gran capacidad de cómputo y de tiempo, y en este proyecto se propone una implementación que utiliza técnicas de programación paralela con la finalidad de disminuir el tiempo de ejecución.
La evaluación experimental de los algoritmos paralelos desarrollados permite concluir que el speedup obtenido se aproxima a ser lineal para sistemas de $2 $ x $ 10^5$ partículas. 

Distintos artículos han sido publicados en el transcurso del mismo \cite{3},  \cite{4}. La implementación realizada se utilizó como base para otros trabajos de maestría \cite{19}. Además, el desarrollo realizado ha sido integrado al paquete de simulación numérica ESyS-Particle y a futuro será posible descargarlo como parte del paquete principal por parte de la comunidad.
...
% Begin Principales resultados
% Principales resultados: Incluir / destacar
El análisis experimental realizado demuestra que el algoritmo desarrollado escala de forma lineal con el número de partículas del sistema, así como con el numero de recursos de computo. 
Diferentes estrategias de asignación de trabajo fueron desarrolladas permitiendo reducir el tiempos de computo final y logrando una eficiencia computacional de 0.85 para algomerados de millones de partículas.

%y escala con una ley de potencia inversa de exponente 0.87 con el número de recursos de computo utilizados en el cálculo. El comportamiento del speedup está cerca de ser lineal para sistemas de $2 × 10^5$ partículas.
%
% The algorithm scales linearly with the number of particles in the system, and it scales with an inverse power law (exponent 0.87) with the number of threads used in the computation. The observed speedup is close to linear for systems containing up to 2 × 105 particles.

%\textcolor{red}{Toward that end, we have developed a state-of-the-art parallel algorithm for self-gravitating agglomerates that scales efficiently with the number of particles as well as the number of computational resources}
% End Principales resultados
...
%En este proyecto se desarrolla un algoritmo paralelo para el cálculo de la auto-gravedad en un agregado de partículas que permite abordar eficazmente los aglomerados formados por un número significativamente mayor de partículas que los abordados en la literatura mencionada.

En el transcurso del proyecto se realizaron diversas actividades vinculadas al mismo. En el año 2012 se obtuvo una beca de Iniciación a la Investigación subvencionada por la ANII. En la edición 2013 de Ingeniería Demuestra, el proyecto ganó el primer premio en la categoría \textit{Baja Interpretación Intuitiva}, con un bono premio de USD 300\cite{1}. En agosto de 2014, se participó en el congreso SBD2014 con la presentación \textit{Computing of Self-gravity for Small Solar System Bodies}\cite{2}.
En setiembre 2014, se publicó un artículo en la revista IEEE, bajo el titulo \textit{High-Performance Computing of Self-Gravity for Small Solar System Bodies} \cite{3}. En abril 2016, se publicó un articulo como parte de la participación del congreso ISUM 2015, bajo el título \textit{A parallel multithreading algorithm for self-gravity calculation on agglomerates} \cite{4}
...
% Fue adquirido con fondos del llamado “Fortalecimiento de Equipamientos para la Investigación de la Comisión Sectorial de Investigación Científica" (2008), y está operativo desde 2009. Desde el 2009 ha pasado por varias mejoras, hasta llegar a la estructura actual.

% cuyos nodos y sus características se presentan en la siguiente tabla.

% \begin{center}
%   \begin{tabular}{ |c|c|c|c|c| }
%     \hline
%     Cantidad & hostname & clase & cores & GB RAM total \\ 
%     \hline
%     8 & node02-09 & 0 & 8-64 & 8-64 \\  
%     4 & node20-23 & 1 & 8-32 & 24-96 \\
%     3 & node30-32 & 2 & 24-72 & 24-72 \\
%     6 & node33-38 & 3 & 24-144 & 72-432 \\
%     1 & tesla & GPU & 8-8 & 48-48 \\
%     Totales: & 22 & nodos & 320 & 712 \\
%     \hline
%   \end{tabular}
% \end{center}

% Como muestra la tabla, el clúster FING cuenta con 320 procesadores y 712 giga bytes de memoria RAM. Los nodos presentados en la tabla según su clase tienen las siguientes características:
% \begin{itemize}
% 	\item clase 0: Dos procesadores Quad core Xeon serie E5430 de 2.66Ghz, 1.333Ghz FSB.
% 	\item clase 1: HP Proliant DL180 G6, con dos procesadores Quad core Xeon serie E5520 de 2.26Ghz, 1.066Ghz FSB.
% 	\item clase 2: HP Proliant DL180 G6, con dos procesadores AMD Opteron 6172, cada uno con 12 cores de 2.1Ghz, 1.066GHz FSB.
% 	\item clase 3: Proliant DL385 G7, con dos procesadores AMD Opteron 6172, cada uno con 12 cores de 2.1Ghz, salvo una que tiene procesadores 6174 de iguales prestaciones.
% 	\item gpu: GPU server con dos procesadores Quad Core Intel Xeon serie E5530, de 2.66Ghz, 48GB de RAM y cuatro tarjetas nVidia Tesla C1060 de 240 núcleos de 1.33Ghz y 4GB de RAM cada una, totalizando 960 cores y 16GB de RAM. 
% \end{itemize}
    
% Asimismo, el clúster FING cuenta con un servidor de acceso de las mismas características que los nodos de la clase 0, que funciona como servidor de archivos y servidor de base de datos, además de ser el punto de entrada para los usuarios, para comunicarse con el gestor y enviar trabajos para su ejecución. Los nodos están conectados en red Gigabit Ethernet por medio de un switch Dell Power Connect, de 24 bocas. Dispone también de un switch KVM y una consola para administración, así como de una UPS que permite cierto nivel de tolerancia a caídas de energía. El sistema operativo que corren los nodos es Centos 5.2. El clúster FING tiene su sitio web http://www.fing.edu.uy/cluster, donde puede encontrarse información actualizada de su composición, así como información en tiempo real de estado. También hay disponibles online en el sitio tutoriales sobre cómo utilizar la infraestructura, por lo que aquí no se profundiza en este tema. 
