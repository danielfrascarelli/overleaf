...
Los objetos que orbitan el Sol se clasifican en planetas, planetas enanos y cuerpos menores del Sistema Solar (Small Solar System Bodies, SSSB) \cite{13}. A diferencia de los planetas y planetas enanos, cuya masa es suficiente para que la autogravedad supere la resistencia mecánica del material y conduzcan al equilibrio hidrostático, los SSSB presentan estructuras internas heterogéneas \cite{14}. Estas estructuras pueden incluir cuerpos monolíticos, objetos fracturados y aglomerados gravitacionales.

Diversas evidencias observacionales indican que una proporción significativa de asteroides y cometas puede describirse como aglomerados granulares auto-gravitantes en fase densa. Entre los indicios más relevantes se encuentran: períodos de rotación cercanos al límite de inestabilidad rotacional \cite{16}, procesos de fragmentación asociados a efectos de marea al atravesar el límite de Roche \cite{17,18}, cadenas de cráteres vinculadas a eventos previos de ruptura y valores de densidad aparente compatibles con estructuras porosas. Observaciones directas, como las obtenidas sobre el asteroide 25143 Itokawa por la misión Hayabusa \cite{15}, refuerzan la hipótesis de que estos cuerpos pueden modelarse como conjuntos de partículas cohesionadas predominantemente por su autogravedad.

Desde el punto de vista numérico, los medios granulares se modelan frecuentemente mediante el Método de Elementos Discretos (Discrete Element Method, DEM), que representa el sistema como un conjunto de partículas que interactúan a través de fuerzas de contacto. En este enfoque, la dinámica macroscópica emerge de la integración temporal de las interacciones microscópicas entre partículas individuales. El DEM ha demostrado ser adecuado para estudiar el comportamiento mecánico de sistemas granulares en múltiples contextos físicos e ingenieriles.

Sin embargo, una limitación relevante en implementaciones DEM estándar es el tratamiento eficiente de fuerzas de largo alcance. En particular, la incorporación de la interacción gravitatoria entre todas las partículas del sistema introduce un costo computacional elevado. El cálculo directo de la auto-gravedad en un sistema compuesto por $N$ partículas requiere evaluar todas las interacciones par a par, lo que implica una complejidad computacional $O(N^2)$. Para sistemas del orden de $10^5$ partículas o superiores —magnitudes necesarias para representar aglomerados realistas— este costo resulta dominante y limita la escala de simulación alcanzable. Esta situación se vuelve aún más restrictiva considerando que en simulaciones DEM el cálculo gravitatorio debe integrarse en esquemas temporales con pasos del orden de $10^{-4}$ s.

El problema abordado en este trabajo consiste en desarrollar un procedimiento computacional que permita calcular la auto-gravedad en aglomerados granulares densos reduciendo el costo respecto al esquema directo cuadrático. Para ello se adopta una estrategia basada en la superposición de una grilla tridimensional regular y estática sobre el dominio ocupado por las partículas, utilizando las celdas de la grilla como unidades mínimas de procesamiento. Sobre esta estructura se aplica un esquema de aproximación jerárquica mediante el Mass Approximation Distance Algorithm (MADA), que permite reemplazar conjuntos de partículas suficientemente alejadas por una masa equivalente concentrada en su centro de masa, manteniendo el cálculo exacto en la región local relevante.

Este enfoque combina discretización espacial y agrupamiento jerárquico con el objetivo de reducir la cantidad de contribuciones evaluadas en cada punto de cálculo. El algoritmo resultante se implementa utilizando descomposición de dominio en memoria compartida, explotando paralelismo multihilo para mejorar el tiempo de ejecución. La formulación propuesta está orientada específicamente a sistemas densos, donde la distribución espacial de partículas es relativamente homogénea y la utilización de una grilla regular resulta adecuada.

El objetivo general del trabajo es diseñar, implementar y evaluar un algoritmo paralelo para el cálculo eficiente de la auto-gravedad en aglomerados de partículas modelados mediante DEM, reduciendo el costo computacional respecto al cálculo directo $O(N^2)$ mediante discretización en grilla regular y aproximación jerárquica por centros de masa, habilitando la simulación de sistemas de gran escala en tiempos de ejecución compatibles con estudios dinámicos.

El documento se organiza de la siguiente manera. En el Capítulo 2 se presenta el marco teórico, incluyendo el Método de Elementos Discretos, la formulación de la auto-gravedad en sistemas discretos, métricas de desempeño en computación paralela, técnicas de interpolación espacial e integración con ESyS-Particle. El Capítulo 3 describe los antecedentes relevantes para el cálculo gravitatorio eficiente. En el Capítulo 4 se formula el problema físico-computacional y se establecen los objetivos del desarrollo junto con los desafíos asociados. El Capítulo 5 presenta el diseño y la arquitectura de la solución propuesta. En el Capítulo 6 se reporta el análisis experimental. Finalmente, el Capítulo 7 expone las conclusiones, mejoras realizadas y líneas de trabajo futuro.
