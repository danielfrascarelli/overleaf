...
Sánchez y Scheeres \cite{12} utilizaron un enfoque ligeramente diferente: mientras que en un código de árbol la subdivisión del espacio es dinámica y depende de la distribución espacial de las partículas, ellos utilizaron una cuadrícula regular y estática para dividir el espacio. Este enfoque parece ser más adecuado para agregados granulares auto-gravitantes en una fase densa, como un asteroide de escombros. Sin embargo, el código no fue paralelizado y las simulaciones se limitaron a unos pocos miles de partículas:contentReference[oaicite:6]{index=6}.

Para tratar de manera eficiente los aglomerados auto-gravitantes formados por un número mayor de partículas que las abordadas por los esfuerzos anteriores, el algoritmo de cálculo desarrollado en este trabajo incorpora elementos de los enfoques descritos: (i) la sustitución mediante una función de ajuste suave del potencial, (ii) un método de agrupación jerárquica para acelerar el cálculo de auto-gravedad en nodos de la cuadrícula, y (iii) una cuadrícula regular y estática para dividir el espacio:contentReference[oaicite:7]{index=7}.
...
El modelo completo —grilla regular más agrupamiento jerárquico mediante MADA— permite reducir el costo del cálculo respecto al método directo $O(N^2)$. Tal como se reporta en el trabajo IEEE, el algoritmo presenta:

\begin{itemize}
    \item Escalamiento aproximadamente lineal con el número de partículas.
    \item Escalamiento respecto al número de hilos que sigue una ley de potencia inversa con exponente cercano a $0.87$.
\end{itemize}
...
Para estudios dinámicos, el cálculo del campo gravitatorio debe integrarse con la evolución temporal del sistema. Esto introduce un compromiso entre precisión y costo: actualizar el campo gravitatorio con demasiada frecuencia puede dominar el tiempo total de ejecución; hacerlo con muy poca frecuencia puede degradar la fidelidad del modelo. En el enfoque adoptado en la bibliografía utilizada, se plantea además la conveniencia de representar el potencial mediante una función suave de la posición y/o aplicar interpolación espacial, de modo de reutilizar el campo durante varios pasos de integración y amortiguar el costo del recálculo.
