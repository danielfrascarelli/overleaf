...
El modelo adoptado para el cálculo de la auto-gravedad en este trabajo se basa en una aproximación jerárquica orientada a sistemas densos de partículas, como los denominados \textit{rubble piles} (aglomerados granulares mantenidos por auto-gravedad). La formulación se inspira en métodos jerárquicos existentes para el problema de N-cuerpos, pero adaptada a un esquema basado en grilla regular y paralelismo en memoria compartida, tal como fue presentado en el artículo \textit{High Performance Computing of Self-gravity for Small Solar System Bodies}.

El cálculo exacto del potencial gravitatorio sobre una partícula $j$ está dado por:

\[
V_j = \sum_{i \ne j} \frac{G M_i}{\|\mathbf{r}_j - \mathbf{r}_i\|}
\]

donde $G$ es la constante de gravitación universal, $M_i$ la masa de la partícula $i$ y $\mathbf{r}_i$ su posición. Este cálculo directo requiere $O(N^2)$ operaciones por paso temporal, lo cual resulta prohibitivo para sistemas con $N$ del orden de millones de partículas.
...
Para acelerar el cálculo del potencial gravitatorio, se utiliza el método denominado \textbf{Mass Approximation Distance Algorithm (MADA)}, introducido en el trabajo mencionado.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{precisión numérica:} cuantificar el error relativo del potencial gravitatorio computado respecto a un caso con referencia analítica o propiedades de simetría conocidas;
        \item \textbf{eficiencia computacional:} caracterizar el tiempo de ejecución y las métricas de paralelismo (aceleración y eficiencia) al variar el tamaño del problema y la cantidad de hilos utilizados.
    \end{itemize}
\end{enumerate}
...
Finalmente, al considerar instancias con un número muy grande de partículas (del orden de un millón) en un dominio tridimensional, donde las partículas se disponen en contacto entre sí y con los bordes del contenedor, el espacio de configuraciones factibles crece de forma explosiva y el problema se encuadra dentro de la clase NP, lo que refuerza la conveniencia de enfoques heurísticos/constructivos para obtener soluciones de buena calidad en tiempos razonables.
