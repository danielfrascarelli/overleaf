...
La dinámica de cada partícula está gobernada por la segunda ley de Newton, expresada en la ecuación~(\ref{eq:segunda_ley_newton}).
...
La interacción gravitatoria es una fuerza de largo alcance que actúa entre todas las masas del sistema. En el contexto clásico newtoniano, la fuerza gravitatoria ejercida por una partícula $j$ sobre una partícula $i$ se expresa como se indica en la ecuación~(\ref{eq:fuerza_gravitatoria_par}).
...
En un sistema compuesto por $N$ partículas, la fuerza total sobre la partícula $i$ es, según la ecuación~(\ref{eq:fuerza_total_gravitatoria}).
...
Sea un volumen cúbico discretizado mediante una grilla regular de dimensiones $L_x \times L_y \times L_z$, cuyos nodos se denotan por la ecuación~(\ref{eq:nodos_grilla}).
...
El potencial en cada nodo de la grilla se calcula como se indica en la ecuación~(\ref{eq:potencial_grilla}).
...
Este procedimiento debe repetirse para todos los nodos de la malla, lo que implica un costo total proporcional a la expresión indicada en la ecuación~(\ref{eq:complejidad_potencial_grilla}).
...
\red{\textbf{no corresponde al marco teórico}}
...
\subsection{Representación del individuo \red{plural}}
...
Un individuo del algoritmo evolutivo representa una configuración completa del sistema, es decir, el conjunto definido en la ecuación~(\ref{eq:ae_conjunto_configuracion}).

\begin{equation}
	\mathcal{S} = \{(\mathbf{x}_i, r_i)\}_{i=1}^{N}
	\label{eq:ae_conjunto_configuracion}
\end{equation}

\red{no es frase} donde las posiciones $\{\mathbf{x}_i\}$ determinan la disposición espacial de todas las partículas dentro del dominio de trabajo.

\subsection{Función objetivo}

El objetivo del proceso de compactación es maximizar la densidad volumétrica del aglomerado, reduciendo al mínimo el volumen vacío entre partículas. Formalmente, el problema puede expresarse como se indica en la ecuación~(\ref{eq:ae_objetivo_compactacion}).

\begin{equation}
	\max_{\{\mathbf{x}_i\}} \; \rho(\mathcal{S})
	\label{eq:ae_objetivo_compactacion}
\end{equation}

\red{no es frase} donde $\rho(\mathcal{S})$ representa la fracción del volumen ocupado por las partículas respecto al volumen total del dominio considerado.

\subsection{Restricciones geométricas}

El problema de optimización está sujeto a la restricción de no solapamiento entre partículas. Para toda pareja de partículas $i \neq j$, debe cumplirse según la ecuación~(\ref{eq:ae_restriccion_no_solapamiento}).

\begin{equation}
	\| \mathbf{x}_i - \mathbf{x}_j \| \ge r_i + r_j
	\label{eq:ae_restriccion_no_solapamiento}
\end{equation}
...
\paragraph{Tiempo de ejecución}\red{falta el punto luego de todos los paragraph}
...
\paragraph{Speedup}
El speedup se define como la razón entre el tiempo en un recurso y el tiempo en $N$ recursos, según la ecuación~(\ref{eq:speedup}).
\begin{equation}
	S(N) = \frac{T(1)}{T(N)}.
	\label{eq:speedup}
\end{equation}
...
La eficiencia paralela normaliza el speedup respecto a la cantidad de recursos utilizados, según la ecuación~(\ref{eq:eficiencia}).
\begin{equation}
...
Sea un aglomerado compuesto por $N$ partículas, donde cada partícula $i$ posee masa $m_i$ y posición $\mathbf{x}_i \in \mathbb{R}^3$. En el marco de la mecánica newtoniana, la fuerza gravitatoria total ejercida sobre la partícula $i$ está dada por la ecuación~(\ref{eq:fuerza_total_gravitatoria_cap4}).
...
\begin{enumerate}
	...
	\item Incorporar el método \textit{Mass Approximation Distance Algorithm} (MADA)\red{definir las siglas la primera vez que se usan} como mecanismo de aceleración, de manera que:
	      \begin{itemize}
...
En este capítulo se presenta el diseño y arquitectura de la solución. \red{repetido}
