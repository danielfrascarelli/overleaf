...
Diversas evidencias observacionales indican que una fracción significativa de asteroides y cometas posee estructura de aglomerados granulares auto-gravitantes en fase densa. Entre estas evidencias se destacan: (i) períodos de rotación superiores al límite de inestabilidad rotacional \cite{16}; (ii) alteraciones por efectos de marea al atravesar el límite de Roche de cuerpos masivos \cite{17,18}; (iii) cadenas de cráteres asociadas a fragmentaciones previas; y (iv) densidades aparentes bajas compatibles con materiales porosos. Observaciones como las realizadas sobre el asteroide 25143 Itokawa por la misión Hayabusa refuerzan esta hipótesis \cite{15}. 
...
El problema consiste en calcular eficientemente la auto-gravedad en sistemas formados por $N$ partículas que representan aglomerados granulares en fase densa. El cálculo exacto del potencial gravitacional requiere $O(N^2)$ operaciones, lo que resulta inviable para sistemas con millones de partículas y pasos de integración del orden de $10^{-4}$ s, como en simulaciones DEM.

Para reducir el costo computacional se adopta un enfoque basado en:

\begin{itemize}
    \item División del espacio mediante una grilla tridimensional regular y estática.
    \item Agrupamiento jerárquico de masas.
    \item Aplicación del Mass Approximation Distance Algorithm (MADA) para aproximar la contribución de regiones lejanas.
\end{itemize}

La grilla define celdas como unidad mínima de procesamiento. Las interacciones cercanas se calculan de forma exacta, mientras que las regiones distantes se aproximan mediante la concentración de masa en su centro de masa.

El algoritmo se implementa utilizando descomposición de dominio y paralelización en memoria compartida, permitiendo escalar respecto al número de partículas y al número de recursos de cómputo.


\section{Objetivo}

El objetivo principal de este trabajo es diseñar, implementar y evaluar un algoritmo paralelo para el cálculo eficiente de la auto-gravedad en aglomerados de partículas modelados mediante DEM, reduciendo el costo respecto al cálculo directo $O(N^2)$ mediante discretización en grilla regular y aproximación jerárquica por centros de masa (MADA), habilitando el estudio de sistemas del orden de millones de partículas en tiempos de ejecución viables.

\section{Resultados principales}

El análisis experimental muestra mejoras significativas en tiempo de ejecución al paralelizar el cálculo de auto-gravedad, con buen aprovechamiento de múltiples hilos dentro del rango evaluado. Asimismo, el enfoque basado en grilla y aproximación jerárquica permite abordar tamaños de sistema superiores a los viables con el cálculo directo.
...
El documento se estructura de la siguiente manera. En el Capítulo 2 se presenta el marco teórico, incluyendo el Método de Elementos Discretos (DEM), la formulación de la auto-gravedad en sistemas discretos, métricas de desempeño en computación paralela, técnicas de interpolación espacial e integración con ESyS-Particle. El Capítulo 3 resume los antecedentes y enfoques previos para reducir el costo computacional del cálculo gravitatorio. En el Capítulo 4 se formula el problema físico-computacional y se establecen los objetivos del desarrollo, junto con los principales desafíos asociados. El Capítulo 5 describe el diseño y la arquitectura de la solución propuesta, abarcando la compactación de aglomerados, el esquema MADA con subdivisión multinivel, el modelo de memoria compartida, la interpolación y la integración con ESyS-Particle. En el Capítulo 6 se reporta el análisis experimental, incluyendo la evaluación de errores y el estudio de escalabilidad (tiempo de ejecución, speedup y eficiencia) bajo distintas configuraciones. Finalmente, el Capítulo 7 presenta las conclusiones, las mejoras realizadas y las líneas de trabajo futuro.
